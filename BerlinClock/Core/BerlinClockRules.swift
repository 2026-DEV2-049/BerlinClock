...
public enum BerlinClockRules {
    ...
    public static func berlinClock(from date: Date) -> BerlinClock {
        ...
        let seconds = SecondsConverter.convert(calendar.component(.second, from: date))
        
        let minutes = calendar.component(.minute, from: date)
        let singleMinutes = MinuteConverter.singleMinute(from: minutes)
        let fiveMinutes = MinuteConverter.fiveMinutes(from: minutes)
        
        let hours = calendar.component(.hour, from: date)
        let singleHour = HourConverter.singleHour(from: hours)
        let fiveHours = HourConverter.fiveHour(from: hours)
        
        return BerlinClock(seconds: seconds, singleMinutes: singleMinutes, fiveMinutes: fiveMinutes, singleHours: singleHour, fiveHours: fiveHours)
    }
